...
fs.readJson(<span class="hljs-string">'./package.json'</span>, (err, packageObj) =&gt; {
  <span class="hljs-keyword">if</span> (err) <span class="hljs-built_in">console</span>.error(err)

  <span class="hljs-built_in">console</span>.log(packageObj.version) <span class="hljs-comment">// =&gt; 0.1.3</span>
})

<span class="hljs-comment">// Promise Usage</span>
fs.readJson(<span class="hljs-string">'./package.json'</span>)
.then(<span class="hljs-function"><span class="hljs-params">packageObj</span> =&gt;</span> {
  <span class="hljs-built_in">console</span>.log(packageObj.version) <span class="hljs-comment">// =&gt; 0.1.3</span>
})
...
<hr>
<p><code>readJson()</code> can take a <code>throws</code> option set to <code>false</code> and it won't throw if the JSON is invalid. Example:</p>
<pre><code class="hljs css language-js"><span class="hljs-keyword">const</span> fs = <span class="hljs-built_in">require</span>(<span class="hljs-string">'fs-extra'</span>)

<span class="hljs-keyword">const</span> file = <span class="hljs-string">'/tmp/some-invalid.json'</span>
<span class="hljs-keyword">const</span> data = <span class="hljs-string">'{not valid JSON'</span>
fs.writeFileSync(file, data)

fs.readJson(file, { <span class="hljs-attr">throws</span>: <span class="hljs-literal">false</span> }, (err, obj) =&gt; {
  <span class="hljs-keyword">if</span> (err) <span class="hljs-built_in">console</span>.error(err)

  <span class="hljs-built_in">console</span>.log(obj) <span class="hljs-comment">// =&gt; null</span>
})

<span class="hljs-comment">// Promise Usage</span>
fs.readJson(file, { <span class="hljs-attr">throws</span>: <span class="hljs-literal">false</span> })
.then(<span class="hljs-function"><span class="hljs-params">obj</span> =&gt;</span> {
  <span class="hljs-built_in">console</span>.log(obj) <span class="hljs-comment">// =&gt; null</span>
})
.catch(<span class="hljs-function"><span class="hljs-params">err</span> =&gt;</span> {
  <span class="hljs-built_in">console</span>.error(err) <span class="hljs-comment">// Not called</span>
})
</code></pre>
