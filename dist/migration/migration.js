...
// it.
var DefaultMigration = { version: 0, up: function up() {} };
...
var Migrations = exports.Migrations = {
  ...
  config: function config(opts) {
    ...
    MongoConnect(this.options.dbUrl).then(function (error, db) {
      _this._collection = db.collection(_this.options.collectionName);
      _this._db = db;
...
Migrations.add = function (migration) {
  if (typeof migration.up !== 'function') throw new Error('Migration must supply an up function.');

  if (typeof migration.version !== 'number') throw new Error('Migration must supply a version number.');

  if (migration.version <= 0) throw new Error('Migration version must be greater than 0');
...
Migrations._migrateTo = function (version, rerun) {
  ...
  function lock() {
    ...
    return Promise.coroutine(regeneratorRuntime.mark(function _callee() {
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _context.next = 2;
              return self._collection.update({ _id: 'control', locked: false }, { $set: { locked: true, lockedAt: new Date() } });
...
Migrations._getControl = function () {
  var control = Promise.coroutine(regeneratorRuntime.mark(function _callee2() {
...
Migrations._setControl = function (control) {
  ...
  this._collection.update({ _id: 'control' }, { $set: { version: control.version, locked: control.locked } }, { upsert: true });
...
Migrations._reset = function () {
  this._list = [{ version: 0, up: function up() {} }];
